#!/bin/bash
# ============================================================
# EC2 초기 설정 스크립트 — Ubuntu 22.04 LTS
...
cp /opt/portal/../deploy/portal.service /etc/systemd/system/portal.service 2>/dev/null || \
...
echo "▶ 8. SQL Server 2022 Express 설치"
...
echo ""
echo "  SQL Server SA 비밀번호를 설정합니다."
MSSQL_PID="Express" /opt/mssql/bin/mssql-conf setup
systemctl enable mssql-server
systemctl start mssql-server

echo "▶ 9. SQL Server 클라이언트 도구(sqlcmd) 설치"
...
echo ""
echo "  2. portal_db 데이터베이스 생성:"
echo "     sqlcmd -S localhost -U SA -P 'YOUR_SA_PW' \\"
echo "       -Q \"CREATE DATABASE portal_db\""
echo ""
...
echo "  5. 배포 후 Swagger UI: http://$(curl -s ifconfig.me)/swagger-ui/index.html"
